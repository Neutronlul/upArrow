...
void addManyReactions(const std::vector<std::string> &emoji, dpp::cluster &bot,
                      const dpp::message &message, int index = 0) {
  if (index < emoji.size()) {
    bot.message_add_reaction(message, emoji[index],
                             [emoji, &bot, message, index](
                                 const dpp::confirmation_callback_t &callback) {
                               if (!callback.is_error()) {
                                 addManyReactions(emoji, bot, message,
                                                  index + 1);
                               }
                             });
  }
...
int main() {
  ...
  bot.on_message_create([&bot](const dpp::message_create_t &event) {
    if (event.msg.author.id == TARGET_USER_ID && !event.msg.content.empty()) {
      bot.message_create(
          dpp::message(TARGET_CHANNEL_ID,
                       gAPICall(event.msg.content) + event.msg.get_url()));
    }
  });
